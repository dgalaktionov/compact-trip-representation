...
namespace cds_static
{
	...
	class Sequence
	{
		...
		public:
            ...
            /*
             * Returns the number of c's in S[start,end]
             */
            ...
            /**
             * Returns the ith occurrence of c in the interval [start,min(end,length-1)]
             * Returns (size_t)-1 if:
             *      --> end<start
             *      --> if the ith occ is beyond the endpoint "end"
             */
			...
			*/
			...
			virtual ulong topK(uint k, uint xs, uint xe, uint *res) const;
			virtual ulong topK(uint k, uint *res) const;

		protected:
			size_t length;
			uint sigma;
			uint buildBestBitmap(vector<BitSequenceBuilder*> &, uint *bmp, size_t len, BitSequence *&bs);
			uint buildBestSequence(vector<SequenceBuilder*> &, uint *bmp, size_t len, Sequence *&bs);
