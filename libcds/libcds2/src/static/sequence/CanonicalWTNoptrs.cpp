...
void build_wt(uint *symbols, int n, uint level, uint maxLevel, uint *sizeBitmaps, uint *bitmaps, tuple *table_symbols, uint *finishAtLevel){
	...
	for (uint i=0;i<n;i++){
		if (!(bitget(&(table_symbols[symbols[i]].code), table_symbols[symbols[i]].len-level-1)) && (table_symbols[symbols[i]].len > level)){
			...
			bit_set(bitmaps, sizeBitmaps[level]);
		}
		sizeBitmaps[level]++;
	} 
...
void build_wt(uint *symbols, int n, uint level, uint maxLevel, uint *sizeBitmaps, uint **bitmaps, tuple *table_symbols, uint *finishAtLevel){
	...
	for (uint i=0;i<n;i++){
		if (!(bitget(&(table_symbols[symbols[i]].code), table_symbols[symbols[i]].len-level-1)) && (table_symbols[symbols[i]].len > level)){
			...
			bit_set(bitmaps[level], sizeBitmaps[level]);
		}
		sizeBitmaps[level]++;
	} 
	uint *symbLeft, *symbRight, pl=0, pr=0;
 ...
 * */
CanonicalWTNoptrs::CanonicalWTNoptrs(uint * _symbols, uint _n, vector<BitSequenceBuilder *> &bsbs, Mapper * am,
	...
	build(symbols,max_v,wcc, bsbs, am, msb);
	//~ delete [] symbols;
}
 ...
 * */
CanonicalWTNoptrs::CanonicalWTNoptrs(uint * _symbols, uint _n, wt_coder_huff *wc, vector<BitSequenceBuilder *> &bsbs, Mapper * am, MappingBuilder *msb,  bool deleteSymbols) : Sequence(n){
	n=_n;
	uint *symbols = new uint[n];
	uint max_v=0;
    for (uint i=0;i<n;i++){
 ...
 * */
CanonicalWTNoptrs::CanonicalWTNoptrs(uint * _symbols, uint _n, wt_coder_huff_can *wc, vector<BitSequenceBuilder *> &bsbs, Mapper * am, MappingBuilder *msb,  bool deleteSymbols) : Sequence(n){
	n=_n;
	uint *symbols = new uint[n];
	uint max_v=0;
    for (uint i=0;i<n;i++){
	...
	sigma=max_v;
	build(symbols,max_v,wc, bsbs, am, msb);
	//~ delete [] symbols;
}

void CanonicalWTNoptrs::build(uint *symbols, uint max_v, wt_coder_huff_can *wcc, vector<BitSequenceBuilder *> &bsbs, Mapper * am,  MappingBuilder *msb){
  ...
  for(uint i=0;i<n;i++) occurrences[symbols[i]]++;
		...
		for (uint i=0;i<this->maxCodeLength;i++){
			buildBestBitmap(bsbs,bitmaps[i],sizeBitmaps[i],bitstring[i]);
		}
		free(sizeBitmaps);
	...
	bit_set(oc, 0);
	  for(uint i=0;i<=max_v;i++)
		  bit_set(oc, occurrences[i]);
	bit_set(oc, n);
	  buildBestBitmap(bsbs,oc,n+1,occ);
	  delete [] oc;
