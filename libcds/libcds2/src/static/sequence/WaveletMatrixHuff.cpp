...
//		this->length = n;
//	}
	WaveletMatrixHuff::WaveletMatrixHuff(uint * symbols, size_t n, wt_coder_huff * coder, vector<BitSequenceBuilder *> & bmb, Mapper * am, MappingWMBuilder *wmb, bool deleteSymbols) : Sequence(n) {
		for (auto it:bmb)
			it->use();
...
//		cout << "Minimum length of a code: " << minLength << endl;
		...
		C = new size_t[height];
		for (uint i=0;i<height;i++){
			buildBestBitmap(bmb,_bm[i],bmpLengths[i],bitstring[i]);
			//cerr << "bpc["<<i<<"]:" << bitstring[i]->getSize()*8.0/bitstring[i]->getLength() << endl;
	...
	ulong WaveletMatrixHuff::extract(ulong i, ulong j, vector<uint> &res) const{
//		for (uint i=0;i<sigma;i++){
//			uint code,len;
//			huffModel->getCode(i,code,len);
//			cerr << "symbol: " << i << ", code: " << code << ", len: " << len << endl;
//		}
		return extract(i,j,0,0U,res);
	}


	ulong WaveletMatrixHuff::extract(ulong i, ulong j, uint level, uint code, vector<uint> &res) const{
		if (i==j) return 0;
		if (level>0 && huffModel->finishCode(code,level)){
			uint symb = huffModel->getSymbol(code,level);
		...
		if (rnk0){
			extract(zeros_before_i,zeros_before_i+rnk0,level+1,code | (1<<level),left);
		}
		if (rnk1){
			auto ones_before_i = i-zeros_before_i;
			extract(C[level]+ones_before_i,C[level]+ones_before_i+rnk1,level+1,code,right);
		}
...
//				}
//			}
//			bps +=(bitstring[i]->getSize()*8.0/bitstring[i]->getLength());
//			cerr << "bps[" << i << "]: " << bps << endl;
//			delete [] _bm[i];
//		}
//
