...
namespace cds_static
{
    ...
    size_t wt_node_internal::rng(size_t xs, size_t xe, uint ys, uint ye,
      ...
      size_t bits = sizeof(uint) * 8;
  		int lc,rc;
  		lc=rc=0;
  		int xs0,xe0,xs1,xe1,shift;
  		shift=bits-level-1;
      current<<=1;
      xs0=bitmap->rank0(xs-1);
      xe0=bitmap->rank0(xe);
      xs1=xs-xs0;
      xe1=xe-(xe0-1);
    ...
    int wt_node_internal::topKSplit(uint xs, uint xe, uint *res,
        std::priority_queue<BinTopK_Item, std::vector<BinTopK_Item>, TopK_Cmp> *topKQueue) const {
        size_t zeros_start = 0;
        size_t zeros_end = bitmap->countZeros();
        size_t ones_start = 0;
        size_t ones_end = bitmap->countOnes();

        if (xs > 0) {
          zeros_start = bitmap->rank0(xs-1);
          ones_start = xs-zeros_start;
        }

        if (xe+1 < bitmap->getLength()) {
          zeros_end = bitmap->rank0(xe);
          ones_end = xe-zeros_end+1;
        }

        if (left_child && (zeros_end > zeros_start)) {
          BinTopK_Item left_item = {zeros_start, zeros_end-1, left_child};
          topKQueue->push(left_item);
        }

        if (right_child && (ones_end - ones_start)) {
          BinTopK_Item right_item = {ones_start, ones_end-1, right_child};
          topKQueue->push(right_item);
        }
