...
namespace cds_static
{
	...
	SequenceBuilderWaveletMatrixHuff::SequenceBuilderWaveletMatrixHuff(wt_coder_huff *coder, vector<BitSequenceBuilder *> &bsb, Mapper * am, MappingWMBuilder *wmb) {
		this->bsb = bsb;
		this->am = am;
		this->wc = coder;
		this->wmb = wmb;
		for (auto it:bsb) it->use();
		am->use();
		wc->use();
	}

	SequenceBuilderWaveletMatrixHuff::~SequenceBuilderWaveletMatrixHuff() {
		for (auto it:bsb) it->unuse();
		am->unuse();
		wc->unuse();
	}
