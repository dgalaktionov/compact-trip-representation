 ...
 *
 * Streategies to obtain the base sequence from a LZ Parsing. Adds to the base those whose length is <=samplingStep 
 *
...
namespace cds_static
{
	...
	LZBaseSampleOnN::LZBaseSampleOnN(uint samplingStep, uint phraseLength){
		this->samplingStep = samplingStep;
		this->phraseLength = phraseLength;
	}
	/** Destroys the Wavelet Tree */
	LZBaseSampleOnN::~LZBaseSampleOnN(){

	}


	uint LZBaseSampleOnN::computeBase(uint *input, size_t length,  SequenceLZEndBase *seq){
		...
		for (uint i=0;i<seq->z;i++){
			if (lengthCounter+seq->phraseLengths[i]<samplingStep && (seq->phraseLengths[i]>phraseLength)){
				lengthCounter+=seq->phraseLengths[i]+1;
			}else if (seq->phraseLengths[i]==0){
				lengthCounter++;
