...
/*
 * Copyright (C) 2015-current_year, Alberto Ordóñez, all rights reserved.
 *
 * Alberto Ordóñez <alberto.ordonez@udc.es>
 *
 * Implementation of Tabei's et al. technique to represent dictionaries.
 * 
...
namespace cds_static{
    ...
    DictionaryRPTabei::DictionaryRPTabei(){}

    DictionaryRPTabei::DictionaryRPTabei(uint *LEFT, uint *RIGHT, uint alph, int R, uint *&PERM):DictionaryRP(R,alph){
        permutation=NULL;
    ...
    DictionaryRPTabei::DictionaryRPTabei(uint *LEFT, uint *RIGHT, uint alph, int R, uint *&PERM,PermutationBuilder *pmb):DictionaryRP(R,alph){
        permutation=NULL;
        buildTrees(LEFT,RIGHT,alph,R,PERM,pmb);
        assert(testDictionaryRP(LEFT,RIGHT,alph,R,PERM));
    }

    DictionaryRPTabei::DictionaryRPTabei(Repair *rp,uint *&PERM,PermutationBuilder *pmb):DictionaryRP(){
        permutation=NULL;
        uint *LEFT,*RIGHT;
    ...
    void DictionaryRPTabei::buildTrees(uint *LEFT, uint *RIGHT, uint ALPH,  int R,  uint *&PERM, PermutationBuilder *pmb){
        int alph;
        alpha=ALPH-1;
        alph=ALPH;
        alphaPlus1=ALPH;
    ...
    uint DictionaryRPTabei::invertUnary(uint val, uint ini, uint len){
        uint res=0;
    ...
    void DictionaryRPTabei::deleteStructures(TreePointers<int> *t){
        if (t==NULL) return;
        if (t->getNChildren()==0)
            delete t;
    ...
    void DictionaryRPTabei::getLOUDS(TreePointers<int> **queue,uint &head,uint &pq, uint &p, uint *louds){
    ...
    void DictionaryRPTabei::getPERM(TreePointers<int> *t, uint &p, uint *perm){
        if (t->getNChildren()==0){
            return;
        }
        for (int i=0;i<t->getNChildren();i++){
    ...
    uint DictionaryRPTabei::getRight(size_t rule){
        assert(rule>alpha);
        return permutation->pi(getParentLOUDS(permutation->revpi(rule),RT_LOUDS));
    }

    uint DictionaryRPTabei::getLeft(size_t rule){
        assert(rule>alpha);
        return getParentLOUDS(rule,LT_LOUDS);
    }

    size_t DictionaryRPTabei::getParentLOUDS(uint r, BitSequence *louds){
    ...
    size_t DictionaryRPTabei::getSize(){
        size_t size = sizeof(DictionaryRPTabei);
    ...
    void DictionaryRPTabei::save(ofstream &of){
        saveValue(of, alpha);
        LT_LOUDS->save(of);
        RT_LOUDS->save(of);
        permutation->save(of);
    ...
    DictionaryRPTabei *DictionaryRPTabei::load(ifstream &in){
        DictionaryRPTabei *d = new DictionaryRPTabei();
    ...
    size_t DictionaryRPTabei::getSizeVerbose(){
        size_t size = sizeof(DictionaryRPTabei);
        size+=LT_LOUDS->getSize();
        size+=RT_LOUDS->getSize();
        if (permutation!=NULL)
    ...
    void DictionaryRPTabei::printTree(TreePointers<int> *t){
